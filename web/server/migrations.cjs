...
const migrations = [
  ...
  {
    name: '004_create_guardian_requests',
    up(db) {
      db.exec(`
        ...
        CREATE INDEX IF NOT EXISTS idx_gr_to ON guardian_requests(to_user_id, status);
        CREATE INDEX IF NOT EXISTS idx_gr_from ON guardian_requests(from_user_id, status);
      `);
    },
  },
];
